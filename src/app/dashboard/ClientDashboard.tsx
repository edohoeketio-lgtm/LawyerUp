...
export default function ClientDashboard({ user }: { user: User | null }) {
    ...
    // Local user state for bookmark updates
    ...
    // Filter Bookings (Clients only see Consultations mostly, but let's show all for them)
    // Actually in original code: return b.type === "consultation"; was for client logic if view was not lawyer.
    const relevantBookings = bookings.filter(b => b.type === "consultation");
    ...
    const toggleBookmark = (lawyerId: string) => {
        if (!localUser) return;
        const currentBookmarks = localUser.bookmarkedLawyerIds || [];
        ...
        setLocalUser({ ...localUser, bookmarkedLawyerIds: newBookmarks });
    };
        ...
        <div className="space-y-8">
            ...
            <div>
                <h2 className="font-serif text-3xl font-medium text-black">
                    {greeting} {localUser ? localUser.firstName : "Guest"}
                </h2>
            ...
            {showProfileCard && (() => {
                const hasLanguageAndTimezone = localUser?.languages && localUser?.languages.length > 0 && localUser?.timezone;
                const hasLegalInterests = localUser?.legalInterests && localUser?.legalInterests.length > 0;
                            ...
                            <div className="relative flex items-center gap-3 pt-6">
                                {hasLegalInterests ? (
                                    <>
                                        ...
                                        <span className="font-medium text-[#8F6B20] line-through decoration-2">Set your preferred legal topics</span>
                                    </>
                                ) : (
                                    <>
                                        <div className="h-5 w-5 rounded-full border-2 border-[#8F6B20] bg-[#FFF8EB] z-10"></div>
                                        <span className="font-medium text-[#4A3B18]">Set your preferred legal topics</span>
                                    </>
                                )}
                            </div>
                    ...
                    <div className="mt-8 text-center border-t border-gray-50 pt-6">
                        {nextBooking && (
                            <div className="inline-flex items-center gap-4 text-left p-3 rounded-xl bg-[#F2FFF2] border border-green-100">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-[#004d45]">Upcoming: Consultation</p>
                                    <p className="text-[10px] text-gray-500">{new Date(nextBooking.date).toLocaleDateString()} • {nextBooking.time} with {nextBookingLawyer?.name}</p>
                                </div>
                                <button className="ml-2 rounded-lg bg-[#004d45] px-3 py-1.5 text-[10px] font-medium text-white hover:bg-[#003a34]">Join</button>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {lawyers
                        ...
                        .map((lawyer) => (
                            <LawyerCard
                                key={lawyer.id}
                                lawyer={lawyer}
                                isBookmarked={localUser?.bookmarkedLawyerIds?.includes(lawyer.id)}
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div className="space-y-4">
                    ...
                    <div className="flex h-48 flex-col items-center justify-center rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100 text-center">
                        <div className="mb-3 rounded-lg bg-gray-100 p-3 grayscale opacity-50"><span className="text-2xl">🏰</span></div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between h-7"></div>
                    <div className="relative h-48 overflow-hidden rounded-xl bg-[#F2FFF2] p-6">
                        <div className="relative z-10 max-w-[60%] space-y-2">
                            ...
                            <p className="text-xs text-gray-600">Help a friend or loved one find the guidance they need.</p>
                            <button className="text-xs font-medium text-[#006056] underline hover:text-[#004d45]">Share invite</button>
                        </div>
                    </div>
                </div>
            </div>
